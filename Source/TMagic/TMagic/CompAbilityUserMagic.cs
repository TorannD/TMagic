    ...
    public class CompAbilityUserMagic : CompAbilityUser
    {
            ...
            if (settingsRef.autocastEnabled && this.Pawn.jobs != null && this.Pawn.CurJob != null && this.Pawn.CurJob.def != TorannMagicDefOf.TMCastAbilityVerb && this.Pawn.CurJob.def != TorannMagicDefOf.TMCastAbilitySelf && this.Pawn.CurJob.def != JobDefOf.Ingest && this.Pawn.CurJob.def != JobDefOf.ManTurret && this.Pawn.GetPosture() == PawnPosture.Standing)
            {
                ...
                if (this.Pawn.drafter != null && !this.Pawn.Drafted && this.Mana != null && this.Mana.CurLevelPercentage >= settingsRef.autocastMinThreshold)
                {
                    ...
                    if (this.spell_TransferMana || isCustom)
                    {
                        ...
                        if (magicPower.autocast && !this.Pawn.CurJob.playerForced && magicPower.learned)
                        {
        ...
        public override void PostExposeData()
        {
            ...
            if (flag11)
            {
                ...
                if (index >= 0)
                {
                    for (int i = 0; i < TM_ClassUtility.CustomClasses()[index].classMageAbilities.Count; i++)
                    {
                        if (TM_ClassUtility.CustomClasses()[index].classMageAbilities[i].shouldInitialize)
                        {
                            for (int j = 0; j < this.MagicData.AllMagicPowersWithSkills.Count; j++)
                            {
                                if (this.MagicData.AllMagicPowersWithSkills[j].abilityDef == TM_ClassUtility.CustomClasses()[index].classMageAbilities[i])
                                {
