    ...
    public class CompAbilityUserMagic : CompAbilityUser
    {
        ...
        public void InitializeSpell()
        {
            ...
            {             
                ...
                {
                    MagicPower BloodMoonPower = this.MagicData.magicPowerBM.FirstOrDefault((MagicPower x) => x.abilityDef == TorannMagicDefOf.TM_BloodMoon);
                    if (BloodMoonPower == null)
                    {
                        BloodMoonPower = this.MagicData.magicPowerBM.FirstOrDefault((MagicPower x) => x.abilityDef == TorannMagicDefOf.TM_BloodMoon_I);
                        if (BloodMoonPower == null)
                        {
                            BloodMoonPower = this.MagicData.magicPowerBM.FirstOrDefault((MagicPower x) => x.abilityDef == TorannMagicDefOf.TM_BloodMoon_II);
                            if (BloodMoonPower == null)
                            {
                                BloodMoonPower = this.MagicData.magicPowerBM.FirstOrDefault((MagicPower x) => x.abilityDef == TorannMagicDefOf.TM_BloodMoon_III);
                            }
                        }
                    }
        ...
        {
            ModOptions.SettingsRef settingsRef = new ModOptions.SettingsRef();
            if (settingsRef.autocastEnabled && this.Pawn.CurJob != null && this.Pawn.CurJob.def.defName != "TMCastAbilityVerb" && this.Pawn.CurJob.def.defName != "Ingest" && this.Pawn.GetPosture() == PawnPosture.Standing)
            {
                //Log.Message("pawn " + this.Pawn.LabelShort + " current job is " + this.Pawn.CurJob.def.defName);
                //non-combat (undrafted) spells
                bool castSuccess = false;
                if (!this.Pawn.Drafted && this.Mana.CurLevelPercentage >= settingsRef.autocastMinThreshold)
                {
                    if (this.Pawn.story.traits.HasTrait(TorannMagicDefOf.Summoner))
                    {
                        ...
                        if (magicPower != null && magicPower.autocast && magicPower.learned && this.summonedMinions.Count() < 4)
                        {
                    ...
                    if (this.Pawn.story.traits.HasTrait(TorannMagicDefOf.Arcanist))
                    {
                        ...
                        foreach (MagicPower current in this.MagicData.MagicPowersA)
                        {
                            ...
                            {                                
                                if ((current.abilityDef == TorannMagicDefOf.TM_Summon || current.abilityDef == TorannMagicDefOf.TM_Summon_I || current.abilityDef == TorannMagicDefOf.TM_Summon_II || current.abilityDef == TorannMagicDefOf.TM_Summon_III))
                                {
                    ...
                    {
                        MagicPower magicPower = this.MagicData.MagicPowersS.FirstOrDefault<MagicPower>((MagicPower x) => x.abilityDef == TorannMagicDefOf.TM_SummonMinion);
                        if (magicPower.autocast && this.summonedMinions.Count() < 4)
                        {
                            PawnAbility ability = this.AbilityData.Powers.FirstOrDefault((PawnAbility x) => x.Def == TorannMagicDefOf.TM_SummonMinion);
                            AutoCast.CastOnSelf.Evaluate(this, TorannMagicDefOf.TM_SummonMinion, ability, magicPower, out castSuccess);
                            if (castSuccess) goto AutoCastExit;
                ...
                if (this.Pawn.Drafted && this.Mana.CurLevelPercentage >= settingsRef.autocastCombatMinThreshold && this.Pawn.CurJob.def != JobDefOf.Goto)
                {
                    ...
                    if (this.Pawn.story.traits.HasTrait(TorannMagicDefOf.HeartOfFrost) && !this.Pawn.story.WorkTagIsDisabled(WorkTags.Violent))
                    {
                        PawnAbility ability = null;
                    ...
                    if (this.Pawn.story.traits.HasTrait(TorannMagicDefOf.Arcanist) && !this.Pawn.story.WorkTagIsDisabled(WorkTags.Violent))
                    {
                        PawnAbility ability = null;
                        foreach (MagicPower current in this.MagicData.MagicPowersA)
                        {
                    ...
                    if (this.Pawn.story.traits.HasTrait(TorannMagicDefOf.Succubus) && !this.Pawn.story.WorkTagIsDisabled(WorkTags.Violent))
                    {
                        PawnAbility ability = null;
                    ...
                    if (this.Pawn.story.traits.HasTrait(TorannMagicDefOf.Warlock) && !this.Pawn.story.WorkTagIsDisabled(WorkTags.Violent))
                    {
                        PawnAbility ability = null;
        ...
        public override void PostExposeData()
        {
            ...
            if (flag11)
            {
                ...
                bool flag50 = abilityUser.story.traits.HasTrait(TorannMagicDefOf.Succubus);
                if (flag50)
                {
                ...
                bool flag51 = abilityUser.story.traits.HasTrait(TorannMagicDefOf.Warlock);
                if (flag51)
                {
                ...
                bool flag55 = abilityUser.story.traits.HasTrait(TorannMagicDefOf.Enchanter);
                if (flag55)
                {
                ...
                if (flag50)
                {
                    ...
                    mpSD = this.MagicData.MagicPowersWD.FirstOrDefault<MagicPower>((MagicPower x) => x.abilityDef == TorannMagicDefOf.TM_SoulBond);
                    if (mpSD.learned == true)
                    {
                        this.AddPawnAbility(TorannMagicDefOf.TM_SoulBond);
                    }
