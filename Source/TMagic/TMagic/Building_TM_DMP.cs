...
namespace TorannMagic
{
    ...
    public class Building_TM_DMP : Building
    {
        ...
        private void TransferMana(CompAbilityUserMagic comp)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                TM_MoteMaker.ThrowGenericMote(ThingDef.Named("Mote_Casting"), comp.Pawn.DrawPos, this.Map, 1.5f - (.4f * i), 0.2f, .02f + (.15f * i), .4f + (.06f * i), Rand.Range(-300, 300), 0, 0, Rand.Range(0, 360)); ;
            }
