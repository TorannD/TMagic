...
namespace TorannMagic
{
    ...
    {
        private int ticksUntilDestruction = 6000;

        public override void CompExposeData()
        {
            base.CompExposeData();
            Scribe_Values.Look<int>(ref this.ticksUntilDestruction, "ticksUntilDestruction", 6000, false);
        }
        ...
        public void DrawEffects()
        {
            ...
            if (this.Pawn.Rotation == Rot4.East)
            {
                ...
                TM_MoteMaker.ThrowGenericMote(ThingDef.Named("Mote_Demon_Flame"), headOffset, this.Pawn.Map, Rand.Range(.5f, .8f), Rand.Range(.12f, .18f), .01f, Rand.Range(.1f, .15f), 0, Rand.Range(2.4f, 2.8f), throwAngle, Rand.Range(0, 360));
            }
            ...
            {
                headOffset.z += .75f;
                TM_MoteMaker.ThrowGenericMote(ThingDef.Named("Mote_Demon_Flame"), headOffset, this.Pawn.Map, Rand.Range(.5f, .8f), Rand.Range(.12f, .18f), .01f, Rand.Range(.1f, .15f), 0, Rand.Range(2.4f, 2.8f), Rand.Range(-30, 30), Rand.Range(0, 360));
            }
            else
            {
                headOffset.z += .75f;
